...
if extension == ".mat":
    ...
    X_inf, _, X_transf = pipeline_signal(
        args.input,
...
# %%
X_inf_rep = se.transform(X_inf.reshape(-1, X_inf.shape[-1])).reshape(X_inf.shape)

# %%
@timing
def predict():
    return model.predict(X_inf_rep)
...
utils.create_dir_hierarchy(
    f'/src/tcc_netro/inference/{model_arch}/classes_{overview["classes"]}/{filename}'
)

if not os.path.isfile(
    f'/src/tcc_netro/inference/{model_arch}/classes_{overview["classes"]}/{filename}/audio.mp3'
):
...
ds = pd.DataFrame(pred_csv, columns=["label", "count", "mean_score"])

ds.to_csv(
    f'/src/tcc_netro/inference/{model_arch}/classes_{overview["classes"]}/{filename}/predictions_{most_pred}p_{overview["segment_length"]}s_{overview["overlap_size"]}o_{overview["augment_size"]}a_{overview["representation"]["n_mfcc"]}c.csv',
    index=False,
)
...
plot_confusion_matrix(confusion.numpy(), size=len(unique_labels), save_path=f"{folder}")
# %%
inf_labels, inf_count = np.unique(y_inf, return_counts=True)

plot_class_distribution(inf_labels, inf_count, save_path=f"{folder}")
# %%
overview = {
    # 'loss': inf_loss,
    # 'accuracy': inf_acc,
